...
class Perceptron:
    ...
    def predict_all(self, data, labels):
        right, wrong = 0, 0
    ...
    def fit_sample(self, training_sample, desired_value):
    ...
    def run(self, training_set, train_labels):
        self.labelset = list(set(train_labels))
        self.labelset.sort()
        for (x, y) in zip(training_set, train_labels):
            self.fit_sample(x, y)
        self.outputter.process(self, training_set, train_labels, self.labelset)
